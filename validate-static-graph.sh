...
case "$target" in
    ...
    host.tools)
        entry_project="Build.proj"
        restore_command=(./build.sh host.tools --restore --runtimeConfiguration "$runtime_configuration")
        dynamic_build_parallelism=(/m:1)
        static_graph_parallelism=(/m:2)
        max_static_graph_nodes=100
        max_static_graph_edges=1000
    ...
    libs.oob)
        ...
        ;;
    *)
        echo "Unsupported static graph validation target '$target'. Supported targets: clr, libs.native, libs.sfx, libs.pretest, libs.tests, libs.oob, host.native, host.tools, host.pretest, host.tests" >&2
        exit 1
        ;;
esac
