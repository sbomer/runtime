...
| --- | --- | --- | --- |
| Build driver | Added the reproducible restore/meta/product driver, package overlays, prerequisite restores, SDK override prebuilds, idempotent bootstrap-target patching, and a runtime-specific private bootstrap copy. | The product graph needs deterministic restored inputs, must reapply changes to generated SDK/package files across bootstrap regenerations, and must not share mutable SDK targets with another runtime precompute environment. | `2c31589f83c`, `3d2ed6864fb`, `0a06753c54d`, `da941d09fc9`, `f4d0a463d07`, `2f319fb8eb1`, `7f293ea5045`, `327fe6f17f7`, `4cf53f6d75f`, `41447c25f9a` |
...
| Deferred producer outputs | Conservatively attach every known output from transitive deferred-state producers to consumers. | Deferred JSON establishes scheduling order but does not authorize BuildXL reads of the producer's explicit file outputs. | `ecc0a497c` |
| Dynamic task state | Preserve raw deferred parameters and record outputs for tasks whose assemblies are loaded only during product execution. | Meta execution cannot expand deferred task parameters or inspect output metadata from an assembly that is not built yet. | `4d769df55` |
